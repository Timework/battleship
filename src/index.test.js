...
describe('tests for the gamedom constructor', function () {

    // this will make two players in the gamedom
    const makeTwoPlayers = () => {
        let testdom = Gamedom();
        testdom.twoPlayer('Chris', 'Mike');
        return testdom.players();
    };
    
    test('the gamedom can make two players if given names', () => {
        expect(makeTwoPlayers()).toBe('Chris, Mike');
    });

    // this will make one player and one computer in the gamedom
    const makeComputerPlayer = () => {
        let testdom = Gamedom();
        testdom.onePlayer('Chris');
        return testdom.players();
    };

    test('the gamedom can make one player and a computer', () => {
        expect(makeComputerPlayer()).toBe('Chris, Computer');
    });
